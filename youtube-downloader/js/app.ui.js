...
function formatListMaker(Obj) {
    ...
    function getItemHTML(data) {
        if (data.audioQuality) {
            return `<div class="quality-item" data-link="${data.url}" data-container=${data.container}>
                <h3>${data.audioQuality.split("_")[2]} QUALITY</h3>
                <div>${(data.contentLength / 1024 / 1024).toFixed(2)}MB · ${data.audioCodec}</div>
            </div>`;
        } else if (data.quality !== null) {
            return `<div class="quality-item" data-link="${data.url}" data-container=${data.container}>
                <h3>${data.qualityLabel}</h3>
                <div>${(data.contentLength / 1024 / 1024).toFixed(2)}MB · ${data.width}x${data.height} · ${data.quality}</div>
            </div>`;
        }
        return "";
    }
